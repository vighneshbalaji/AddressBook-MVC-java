...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into AddressBook values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"','"+person.getAge()+"')";
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"',age='"+person.getAge()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    @Override
    public DefaultListModel getSearchNames(NameListPanel namePanel, String searchNames) {
        DefaultListModel<String> dlm = new DefaultListModel<>();
        ...
        try {
         conn = DBConnection.getInstance().getConnect();
            Statement stmt;
                 stmt = conn.createStatement();
            String qry = "select * from AddressBook where name like '%"+searchNames+"%'";
                 try (ResultSet rs = stmt.executeQuery(qry)) {
                     while (rs.next()){
                         String name = rs.getString(1);
                         dlm.addElement(name);
